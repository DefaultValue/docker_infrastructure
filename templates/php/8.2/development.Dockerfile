...
# Node 18 is EOL and unavailable via nodesource repos, installing from official binary
RUN ARCH=$(dpkg --print-architecture) \
    && if [ "$ARCH" = "amd64" ]; then NODEARCH="x64"; else NODEARCH="arm64"; fi \
    && curl -fsSL "https://nodejs.org/dist/v18.20.8/node-v18.20.8-linux-${NODEARCH}.tar.xz" | tar -xJ -C /usr/local --strip-components=1 \
